...
const MyComponent = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await axios.post(`${config.API_URL}/api/details/add_details`, formData, {
        headers: {
      ...
      console.error('Error submitting game details:', error.response?.data || error.message);
      setError(error.response?.data?.error || 'An error occurred while submitting game details');
    }
  };
  ...
  const fetchGames = async () => {
    try {
      ...
      if (Array.isArray(response.data)) {
        setGames(response.data.map(game => ({ value: game.name, label: game.name })));
      } else {
  ...
  const fetchBonuses = async () => {
    try {
      ...
      if (Array.isArray(response.data)) {
        setBonuses(response.data.map(bonus => ({ value: bonus.name, label: bonus.name })));
      } else {
  ...
  return (
    <div className="p-8 bg-gradient-to-br from-indigo-100 via-purple-100 to-pink-100 rounded-3xl shadow-2xl lg:h-[38vw] overflow-auto">
      <h1 className="text-3xl font-bold text-center mb-8 text-indigo-800">Game Details</h1>
      <form className="mb-4 flex flex-col lg:flex-row gap-8">
        <div className="w-full lg:w-1/2 bg-white p-6 rounded-xl shadow-lg">
          <label className="block mb-2 text-sm font-medium text-gray-700" htmlFor="image-upload">
            Upload Image
          </label>
          ...
          <div className='border-2 border-dashed border-indigo-300 p-2 w-full h-64 rounded-lg overflow-hidden flex items-center justify-center bg-indigo-50'>
            {image && (
              <div className="mb-4">
        ...
        <div className="w-full lg:w-1/2 bg-white p-6 rounded-xl shadow-lg">
          <div className="mb-4">
            <label className="block mb-2 text-sm font-medium text-gray-700" htmlFor="paragraph1">
              Title
            </label>
            <input
              id="paragraph1"
          ...
          <div className="mb-4">
            <label className="block mb-2 text-sm font-medium text-gray-700" htmlFor="paragraph2">
              Description
            </label>
          ...
          <div className='flex flex-col lg:flex-row gap-4 mb-4'>
            <div className='flex flex-col w-full lg:w-1/2'>
              <label className="block mb-2 text-sm font-medium text-gray-700" htmlFor="number1">
                RTP
              </label>
              <input
                id="number1"
            ...
            <div className='flex flex-col w-full lg:w-1/2'>
              <label className="block mb-2 text-sm font-medium text-gray-700" htmlFor="number2">
                Minimum Deposit
              </label>
              <input
                id="number2"
